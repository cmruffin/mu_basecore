  ...
  )
{
  ...
  //
  // Attempt to initialize the md5 hash algorithm, this is done for performance reasons
  // md5 is the fastest hash algorithm supported by the Hash2 Protocol
  // however, if the platform does not support md5, then we will fall back to sha256
  //
  Status = Hash2Protocol->HashInit (Hash2Protocol, &gEfiHashAlgorithmMD5Guid);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_NET, "Failed to initialize md5 hash algorithm: %r\n", Status));

    //
    // slower but most common hash algorithm
    //
