...
// MU_CHANGE TCBZ4542 [BEGIN] - Time based seed generated by NetRandomInitSeed() is not cryptographically strong

/*
Generate a Random output data given a length.

@param Output - The buffer to store the generated random data.
  ...
  )
{
  ...
  Status = RngProtocol->GetRNG (RngProtocol, &gEfiRngAlgorithmSp80090Hash256Guid, OutputLength, (UINT8 *)Output);
  if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  Status = PseudoRandom (&Random, sizeof (Random));
  if (EFI_ERROR (Status)) {
    return Status;
  }
...
// MU_CHANGE TCBZ4542 [END] - Time based seed generated by NetRandomInitSeed() is not cryptographically strong
