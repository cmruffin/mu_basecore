...
//
// These represent algorithms that should be supported by the RNG protocol and are generally considered secure.
// These are the algorithms that should be used to generate the seed for the random number generator.
//
...
//
GLOBAL_REMOVE_IF_UNREFERENCED EFI_GUID  *SecureHashAlgorithms[] = {
  ...
  )
{
  ...
  EFI_STATUS        Status;
  UINTN             SecureHashAlgorithmsSize;

  SecureHashAlgorithmsSize = sizeof (SecureHashAlgorithms) / sizeof (EFI_GUID *);
  ...
  for (UINTN i = 0; i < SecureHashAlgorithmsSize; i++) {
    Status = RngProtocol->GetRNG (RngProtocol, SecureHashAlgorithms[i], OutputLength, (UINT8 *)Output);
    if (!EFI_ERROR (Status)) {
      //
